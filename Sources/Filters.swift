...
func lowerCamelcase(_ value: Any?) -> Any? {
    if let array = value as? [Any?] {
        return array.map { lowerCamelcase($0) }
    }
    let string = stringify(camelcase(value))
    guard string.isEmpty == false else {
        return ""
    }
    return "\(string[string.startIndex].lowercased())\(string[string.index(after: string.startIndex)...])"
}

func upperCamelcase(_ value: Any?) -> Any? {
    if let array = value as? [Any?] {
        return array.map { upperCamelcase($0) }
    }
    let string = stringify(camelcase(value))
    guard string.isEmpty == false else {
        return ""
    }
    return "\(string[string.startIndex].uppercased())\(string[string.index(after: string.startIndex)...])"
}

func camelcase(_ value: Any?) -> Any? {
    if let array = value as? [Any?] {
        return array.map { camelcase($0) }
    }
    ...
    return words[1...].reduce(words[0]) { result, word in
        guard words.isEmpty == false else {
            return result
        }
        return result + "\(word[word.startIndex].uppercased())\(word[word.index(after: word.startIndex)...])"
    }
}
